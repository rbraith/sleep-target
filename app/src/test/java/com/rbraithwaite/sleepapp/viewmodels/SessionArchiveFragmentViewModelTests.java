...
public class SessionArchiveFragmentViewModelTests
{
    ...
    public void getSleepSessionData_nullWhenBadID() {
        ...
        when(mockRepository.getSleepSessionData(badSessionID)).thenReturn(new MutableLiveData<SleepSessionData>(null));

        LiveData<UISleepSessionData> retrievedUILiveData = viewModel.getSleepSessionData(badSessionID);
...
//        when(mockRepository.getSleepSessionData(sessionID)).thenReturn(mockLiveData);
    ...
    public void getAllSleepSessionIds_returnsEmptyListWhenNoIds() {
        when(mockRepository.getAllSleepSessionDataIds()).thenReturn(new MutableLiveData<List<Integer>>(new ArrayList<Integer>()));
