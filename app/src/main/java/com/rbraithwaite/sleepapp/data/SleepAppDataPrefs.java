...
public class SleepAppDataPrefs {
    ...
    public void setCurrentSession(final Context context, final Date startTime) {
        mExecutor.execute(new Runnable() {
            ...
            public void run() {
                ...
                editor.putLong(CURRENT_SESSION_KEY, (startTime == null) ? NULL_VAL : startTime.getTime());
                editor.commit();
    ...
    public LiveData<Date> getCurrentSession(final Context context) {
        ...
        mExecutor.execute(new Runnable() {
            ...
            public void run() {
                Date result = null;
                long currentSessionStartDate = getSharedPrefs(context).getLong(CURRENT_SESSION_KEY, NULL_VAL);
