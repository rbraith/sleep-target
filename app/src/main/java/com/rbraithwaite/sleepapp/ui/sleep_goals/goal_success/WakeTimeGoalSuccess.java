...
//*********************************************************
    ...
    public WakeTimeGoalSuccess(
            List<WakeTimeGoalModel> goalHistory,
...
//*********************************************************
    ...
    private List<Date> computeAllSucceededDates(List<WakeTimeGoalModel> wakeTimeGoalHistory)
    {
        ...
        for (int i = 0; i < wakeTimeGoalHistory.size(); i++) {
            WakeTimeGoalModel current = wakeTimeGoalHistory.get(i);
            if (!current.isSet()) {
                continue;
            }
            ...
            if (notLastElem) {
                WakeTimeGoalModel next = wakeTimeGoalHistory.get(i + 1);
     ...
     */
    private boolean wakeTimeGoalWasMet(SleepSessionModel sleepSession, Date wakeTimeGoal)
    {
